...
class DAQ(ABC):
    ...
    def list_devices(self) -> list[str]:
        """List devices detected by driver
    ...
    def list_ports(self) -> list[str]:
        """List ports detected by self.connected_device
...
class DummyDAQ(DAQ):
    """ Example of a class implementing the DAQ abstract class.
    This class is shown if no other driver is available.
    """
    ...
    def get_sequence(
        ...
        main_window.main_ui.sample_rate_status.setText(
            f"{config['sample_rate_real'].to(ureg.Hz).magnitude:6g}"
        )

        config["signal_range_min_real"] = config["signal_range_min"]
        config["signal_range_max_real"] = config["signal_range_max"]
        main_window.main_ui.range_min_status.setText(
            f"{config['signal_range_min_real'].to(ureg.volt).magnitude:.6g}"
        )
        main_window.main_ui.range_max_status.setText(
            f"{config['signal_range_max_real'].to(ureg.volt).magnitude:.6g}"
        )
        ...
        log.info(
            f"Emit at {average_index + 1}/{averages} - {(time.time() - start_time) * 1e3:.2f} ms"
        )
    ...
    def acquire(self, duration:float, sample_rate:float) -> np.ndarray:
        ...
        s += np.sum(
            decrease[:, np.newaxis] * np.sin(2 * np.pi * rand[:, np.newaxis] * t),
            axis=0,
        )
