...
class MainUI(QWidget):
    ...
    def add_driver_box(self):
        ...
        # Show custom properties button
        # TODO: it needs to be implemented on a per driver basis
    ...
    def add_settings_box(self):
        ...
        self.range_min_edit = QLineEdit(
            placeholderText=str(
                DEFAULT_VALUES["signal_range_min"].to(ureg.volt).magnitude
            )
        )
        ...
        self.range_max_edit = QLineEdit(
            placeholderText=str(
                DEFAULT_VALUES["signal_range_max"].to(ureg.volt).magnitude
            )
        )
    ...
    def add_status_box(self):
        """Box that displays the used settings"""
    ...
    def add_plot_box(self):
        """Box that contains the plotting options"""
        ...
        self.plot_signal_cb.setToolTip(
            "Plot signal diagram only if this is enabled. Disable for faster measurements."
        )
        ...
        self.plot_spectrum_cb.setToolTip(
            "Plot spectrum diagram only if this is enabled. Disable for faster measurements."
        )
    ...
    def calculate_psd_and_plot(self):
        """Calculate the PSD and plot it. Function that is called by pressing button 'Calculate PSD'."""
        ...
        plot_worker.signals.finished.connect(
            lambda: self.main_window.plots.update_plots(
                index=None, force_draw=False, plot_signal=False
            )
        )
    ...
    def plot_signal(self):
        """Plot the latest measured signal. Function that is called by pressing button 'Plot Signal'."""
    ...
    def on_optimized_measurement_changed(self, state):
        """Handle changes to the optimized measurement checkbox."""
        is_enabled = self.optimized_measurement_cb.isChecked()
        
        # Enable/disable the max stored measurements input
        self.max_stored_label.setEnabled(is_enabled)
    ...
    def read_config(self):
        """Read the configuration (what is selected in GUI) from the UI and return it as a dictionary."""
        ...
        # Add optimized measurement setting
    ...
    # several functions to set individual settings in GUI, but can all be done with set_config()
    def set_driver(self, driver: str) -> DAQ:
        """set the driver in the GUI if exists and return the driver instance."""
    ...
    def show_device_properties(
        ...
        """Show the properties of the connected device in a separate window."""
        #TODO: implement this on a per driver basis and maybe also create a 
        # system where this depicted options can be edited.

        if self.driver_instance and self.driver_instance.connected_device:
            self.properties_page = PropertiesWindow(
                parent=self, driver=self.driver_instance
            )
            self.properties_page.show()
            self.properties_page.activateWindow()

    def stop_measurement(self):
        """Function that is called, when one clicks on stop measurement. Aborts measurement."""
    ...
    def finish_measurement(self, length: int):
        """tyding up after measurement is finished."""
        # TODO: stop all plotting and plot final data - I think is implemented, but needs checking
    ...
    def get_data_and_plot(self, index: int):
        ...
        if self.main_window.measurement_stopped:
            log.debug(
                "Wanted to plot data at index {}, but measurement was stopped".format(
                    index
                )
            )
            return
        ...
        plot_worker = Worker(
            self.main_window.data_handler.calculate_data, index, ignore_check=False
        )
        plot_worker.signals.finished.connect(
    ...
    def list_devices(self):
        """List all devices that are connected to the selected driver."""
    ...
    def connect_device(self):
        "updated the GUI and configuration after a device is connected"
        # change Text
        if self.driver_instance.connected_device is None:
    ...
    def update_term_config_dd(self):
        """Update the terminal configuration dropdown as each device has different options."""
        # show modes for input channel
